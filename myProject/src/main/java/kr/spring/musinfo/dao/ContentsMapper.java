...
public interface ContentsMapper {
	...
	@Select("select review, nickname, rev_rate from (select * from reviews r, member_detail m where r.mem_num=m.mem_num and mus_num=#{mus_num} order by rev_regdate desc)where rownum<=2")
